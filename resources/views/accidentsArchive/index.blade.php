...
<div class="row">
    <div class="col align-self-start">
      <a class="btn btn-primary" href="{{route('accidentsArchive.create')}}" role="button">Archiver un accident</a>
    </div>
</div>
</br>
  ...
  <table class="table table-stripped table-info table-borderless">
    <thead class="table bg-gradient-dark table-borderless text-light">
      <tr>
        ...
        <th width='33%'>Accident Archivé</th>
        
        <th width='33%'>Actions</th>
      </tr>
      </thead>
      <tbody class="table-group-divider bg-gradient-dark text-light">
      @foreach ($accidentsArchive as $item)
        <tr class=" table text-light" >
          <td>{{$item->DateArchivage}}</td>
          <td><a class="text-white" href='{{route('accidentsArchive.show',$item->id)}}'>{{$item->AcciArch}}</a></td>
          
          <td class="d-flex x-2">
          

            
            <a class="btn btn-info  bg-gradient-info mr-2" href='{{route('accidentsArchive.show',$item->id)}}'><i class="fas fa-info-circle"></i></a>         
            <form onsubmit="return confirmDelete()" class="mr-2" action='{{route('accidentsArchive.destroy',$item->id)}}' method="post">
            @csrf
            @method('DELETE')
          ...
          function confirmDelete() {
            return confirm('Voulez-vous supprimer cet archive ?');
          }
          </script>
          </td>
